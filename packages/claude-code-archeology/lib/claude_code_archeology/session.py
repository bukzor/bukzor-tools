...
JsonValue = Record | list[Any] | str | int | float | bool | None
"""What a record's fields hold. Content lists are heterogeneous in practice:
malformed and truncated records are normal in this format, so anything read
out of one gets an isinstance check before it is used."""
...
class Session:
    ...
    by_uuid: dict[str, Node] = field(default_factory=dict[str, Node])
...
def parse_jsonl(text_lines: Iterator[tuple[int, str]]) -> Iterator[Node]:
    ...
    for line_no, raw in text_lines:
        ...
        try:
            ...
            print(
                f"warn: skipping malformed JSONL at line {line_no}: {e}",
                file=sys.stderr,
            )
            continue
