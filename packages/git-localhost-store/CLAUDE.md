...
Working directory paths are encoded by `claude_code_slug.path_slug`, a
declared dependency, imported by `cli.py`. Every character that is not
ASCII alphanumeric becomes exactly one `-` -- no squeezing of runs, no
case folding:
```
...
created or recovered -- the relocator short-circuits when `.git` is
already a symlink.

Imported rather than shelled out to, and that is the point of the
Python/bash split described below: a wrong encoder does not fail, it
names a *second* store for a repository that already has one. An import
resolves in the venv this package was installed into; a command name
resolves in whatever PATH the shell, editor or cron daemon exported.

## What lives where

Four files do the work, and the seams between them are all
load-bearing:

| file | role |
|---|---|
| `install.py` | writes the template and the public `bin/` link |
| `hook.sh` | what git copies into a repo; execs the public path |
| `cli.py` | decides *which* store, then hands off |
| `relocate.sh` | drives git: moves, adopts, merges refs |

The venv is not a place git can find anything, so
`git-localhost-store-install` materializes two things outside it: the
template directory `init.templateDir` points at, and
`~/.local/share/git-localhost-store/bin/git-localhost-store`, a symlink
to the console script. That symlink is the system's stable public name.
Every hook copy ever made addresses it -- thousands of them, going back
to 2025 -- so it is what lets a release change the code under repos that
were initialized years earlier.
...
just execs the public path, passing its own invoked name
...
`relocate.sh` itself short-circuits when `.git` is already a symlink or
a gitfile -- the hooks no longer duplicate that check (removed as
redundant once invocation frequency dropped from every ref write to once
per commit/checkout).
...
still-in-flight commit. So `relocate.sh` never runs the "store already
exists" branch when invoked as `post-index-change`, regardless of
...
Answers one question -- which store belongs to this workdir -- and execs
`relocate.sh` with the workdir and store as arguments. Also the
recursion guard: sets `GIT_LOCALHOST_STORE_ACTIVE=1` in the environment
it hands down, and returns immediately if it is already set. Hooks
inherit it from the invoking git process, so the recovery merge's own
fetch/update-ref calls into the store can't recurse through a stale,
pre-2026-07-13 store's real `reference-transaction` hook (new stores
never get one).
...
cleanly on unexpected states). Bash because every line of it is a git
invocation; it computes no paths of its own.
...
`uv run pytest packages/git-localhost-store` drives real git through
real hooks, with every XDG path and the git config redirected into
`tmp_path` — see `lib/git_localhost_store/cli_test.py`. A test that
reached the real store would be editing the repositories this tool
exists to protect, so check that redirection first when adding one.

`docs/dev/testing.kb/` holds the scenarios the harness doesn't cover
yet, one file per scenario, run by hand.
...
1. Edit `hook.sh`.
2. Re-run `git-localhost-store-install` to rewrite the template.
3. Test against a fresh clone.
4. Existing repos retain the hooks copied at clone time — they pick up
   changes only on re-init. What they *do* pick up immediately is a new
   `relocate.sh`, since their copied hook resolves the public symlink at
   each firing. Prefer changing the relocator over changing the hook.

### Debugging Issues

Check in order:
1. Is `init.templateDir` configured? `git config --global init.templateDir`
2. Does the public path resolve?
   `ls -l ~/.local/share/git-localhost-store/bin/git-localhost-store`
   — a dangling link here breaks `git commit` in every relocated repo,
   and is what a moved or deleted venv looks like. Fix:
   `git-localhost-store-install`.
3. Hook output: hooks write to stderr, visible during `git add` / `git
   commit`.
4. Inspect a store: `ls -la ~/.local/state/git-localhost-store/repos/<encoded>/`
5. Check the workdir's `.git`: `ls -la .git` (should be a symlink) and
...
- ❌ **Don't resolve a "store already exists" refusal solo** — that
  ...
  clone) — the common re-clone case now self-heals via the hooks, no
...
Commits before 2026-08-10 are in the `bukzor/dotfiles` repo, where this
lived as `.local/share/git-localhost-store/` — `git log` here starts at
the move. The ADRs and devlogs carry the reasoning across it.
